...
function EditableText({ value, onSave, editMode }: {
  ...
  if (!editMode) {
    return <span>{value || <span style={{ color: '#9ca3af', fontStyle: 'italic' }}>未填写</span>}</span>;
  }
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  const { editMode } = useResumeStore();
  if (!editMode) return <>{children}</>;
  return <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', marginBottom: 6, letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>{children}</div>;
}
...
export default function ResumePreview() {
  ...
  const TemplateComponent = {
    'template-1': Template1Editable,
    'template-2': Template2Editable,
    'template-3': Template3Editable,
    'template-4': Template4Editable,
    'template-5': Template5Editable,
  }[selectedTemplate] || Template1Editable;
    ...
    <div style={{ position: 'relative', height: '100%' }}>
      ...
      {editMode && (
        <div style={{
          ...
          ✏️ 预览编辑模式 — 点击任意文字直接修改，修改会自动同步到左侧表单
        </div>
      )}
      ...
      <div style={{ paddingTop: editMode ? 36 : 0, transition: 'padding-top 0.2s' }}>
        <TemplateComponent
          ...
          patchWork={patchWork}
          patchWorkHighlight={patchWorkHighlight}
          patchEdu={patchEdu}
          patchProj={patchProj}
          patchProjHighlight={patchProjHighlight}
        />
      </div>
    </div>
  );
      ...
      <div className="w-[30%] bg-slate-800 text-white p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold mb-2">
            <EditableText value={p.name} onSave={(v) => patchProfile('name', v)} editMode={editMode} />
          </h1>
          <p className="text-base text-slate-300 mb-3">
            <EditableText value={p.titles?.default || ''} onSave={(v) => patchProfile('titles', { ...p.titles, default: v })} editMode={editMode} />
          </p>
                ...
                <div key={cat.id}>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(cat.skills || []).map((s: any, i: number) => (
                      <span key={i} className="text-xs px-2 py-0.5 bg-slate-700 rounded text-slate-300">{s.name}</span>
                    ))}
                  </div>
                </div>
              ))}
        ...
        {p.summary && (
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-600 leading-relaxed mt-2">
              <EditableText value={p.summary} onSave={patchSummary} editMode={editMode} />
            </p>
          </div>
        )}
              ...
              {(resume.projects || []).map((proj: any, idx: number) => (
                <div key={proj.id}>
                  <div className="flex justify-between items-baseline mb-1">
                    <span className="font-semibold text-slate-900">
                      <EditableText value={proj.name} onSave={(v) => patchProj(idx, 'name', v)} editMode={editMode} />
                    </span>
                    {proj.role && (
                      <span className="text-xs text-slate-400">
                        <EditableText value={proj.role} onSave={(v) => patchProj(idx, 'role', v)} editMode={editMode} />
                      </span>
                    )}
                  </div>
      ...
      {(resume.skills || []).length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {(resume.skills || []).flatMap((cat: any) =>
              (cat.skills || []).map((s: any, i: number) => (
                <span key={i} className="text-slate-600">{s.name}</span>
              ))
            )}
          </div>
        </div>
    ...
    <div className="p-6 bg-orange-50" style={{ minHeight: '297mm', fontFamily: 'system-ui, sans-serif' }}>
      <div className="flex items-center gap-4 mb-4">
        <div className="flex-1">
          ...
          <p className="text-base text-orange-600 font-medium mb-2">
            <EditableText value={p.titles?.default || ''} onSave={(v) => patchProfile('titles', { ...p.titles, default: v })} editMode={editMode} />
          </p>
          ...
          <div className="bg-white rounded-lg p-4">
            ...
            {(resume.skills || []).map((cat: any) => (
              <div key={cat.id} className="mb-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {(cat.skills || []).map((s: any, i: number) => (
                    <span key={i} className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 rounded-full">{s.name}</span>
                  ))}
                </div>
              </div>
            ))}
...
function Template4Editable({ resume, editMode, patchProfile, patchSummary, patchWork, patchWorkHighlight, patchEdu }: any) {
  const p = resume.profile || {};
  const updateCell = (key: string, value: string) => patchProfile(key, value);
  return (
      ...
      <table className="w-full text-xs mb-4 border-collapse">
        <tbody>
          <tr>
            ...
            <td className="border border-slate-800 p-1"><EditableText value={p.name} onSave={(v) => updateCell('name', v)} editMode={editMode} /></td>
          ...
          <tr>
            ...
            <td className="border border-slate-800 p-1"><EditableText value={p.phone || ''} onSave={(v) => updateCell('phone', v)} editMode={editMode} /></td>
            <td className="border border-slate-800 p-1">邮箱</td>
            <td className="border border-slate-800 p-1"><EditableText value={p.email || ''} onSave={(v) => updateCell('email', v)} editMode={editMode} /></td>
          </tr>
        </tbody>
      </table>
      {p.summary && (
      ...
      {(resume.skills || []).length > 0 && (
        <div className="text-xs">
          ...
          <p className="text-slate-600">
            {(resume.skills || []).flatMap((cat: any) => (cat.skills || []).map((s: any) => s.name)).join('、')}
          </p>
        </div>
      )}
    </div>
            ...
            <div>
              ...
              {(resume.skills || []).map((cat: any) => (
                <div key={cat.id} className="mb-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(cat.skills || []).map((s: any, i: number) => (
                      <span key={i} className="text-xs px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full">{s.name}</span>
                    ))}
                  </div>
                </div>
              ))}
...
function computeHealth(resume: any, jdAnalysis: JdAnalysis | null) {
  ...
  return { score: Math.max(0, score), issues: issues.slice(0, 5), jdCoverage };
}
